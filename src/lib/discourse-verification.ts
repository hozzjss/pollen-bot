...
// discord_id of user is Mandatory!  verification_code should only be sent if the user uses the " !{whatever command prefix} discourse_check ${verification_code} " code
...
async function matchUser(
  ...
  if (verification_code !== tempStorage[discord_id]) {
    message = errorDiscourseVerificationEmbed(
      ...
      **!${commandPrefix} verify-discourse ${discourse_username}**`
    );
    ok = false;
